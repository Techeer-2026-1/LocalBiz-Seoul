...
보안 정책 (CodeRabbit #3 학습 적용):
  ...
  - 상세 사유는 logger.info로만 (만료/위조/사용자 미존재 구분).
"""
...
async def login_email(req: LoginRequest) -> TokenResponse:
    ...
    if row is None:
        logger.info("login failed: user not found (email=%s)", req.email)
        raise HTTPException(
    ...
    if row["password_hash"] is None:
        logger.info("login failed: google user attempted password login (email=%s)", req.email)
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail=_LOGIN_ERROR_DETAIL,
        )
    ...
    if not verify_password(req.password, row["password_hash"]):
        logger.info("login failed: wrong password (email=%s)", req.email)
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail=_LOGIN_ERROR_DETAIL,
        )
