"""EVENT_RECOMMEND 노드 — 취향/맥락 기반 행사 추천 + 추천 사유.

검색 흐름 (EVENT_SEARCH와 같은 DB 우선 → Naver fallback 패턴, 단 추천 사유 강조):
  ...
  5. response_blocks: text_stream(추천 사유 강조) + events[] + references[]

EVENT_SEARCH와의 차별화:
  - 시스템 프롬프트가 "추천 이유" 명시 (각 행사별 카테고리 적합성/일정·위치/특징 근거)
